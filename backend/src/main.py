...
async def global_http_logging_middleware(request: Request, call_next):
    ...
    try:
        ...
        duration_ms = (time.time() - start_time) * 1000
        logger.error(f"[GLOBAL HTTP EXCEPTION] [{request_id}] Client: {client_ip}, Method: {request.method}, URL Path: {request.url.path} failed with Exception after {duration_ms:.2f}ms. Error: {e}", exc_info=True)
        raise
