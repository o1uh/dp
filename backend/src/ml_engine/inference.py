...
def separate_track(input_path: Path, output_dir: Path, model_type: str = "htdemucs") -> dict:
    logger.info(f"Starting neural separation pipeline. Source File: {input_path}, Target Output: {output_dir}, Separation Logic: {model_type}")
    ...
    logger.info("Running optimized inference calculations for standard stems...")
    ...
    if model_type == "cascade_guitar":
        logger.info("Cascade guitar mode detected. Loading PyTorch guitar model weights parameters...")
    ...
    logger.info("Neural separation calculations successfully finalized.")
    return results